function I1 = steepest_descent_H(omega,A,B,C,a,b,f)
g = @(x) A*x.^2 + B*x + C;
% f = @(x) exp(-10*x.^2);%1;
% a = -1; b = 1;
xi = -B/2/A;    % stationary points

% SD paths
uin = @(x,p) sqrt((x-xi).^2 + 1i*p/A);
hx = @(x,p) xi + uin(x,p);
hx2 = @(x,p) xi - uin(x,p);
hxp = @(x,p) xi + sqrt(1i/A)*p;

% derivatives
hprim = @(x,p) 1i/2/A./uin(x,p);
hprim2 = @(x,p) -1i/2/A./uin(x,p);
hprimp = sqrt(1i/A/omega);  % hprim*2*u for x = xi
...
% SD
  ...
  F1xi = @(x0) exp(1i*omega*g(x0))*(w2*f(hxp(x0,pj2/sqrt(omega))))*hprimp;
  F2x = @(x0) -omega^(-1-alpha1).*exp(1i*omega*g(x0))*sum(f(hx2(x0,pj1)).*hprim2(x0,pj1).*pj1.^(-alpha1).*w1);
%   F2xi = @(x0) -omega^(-1).*exp(1i*omega*g(x0))*sum(f(hx2(x0,pj2)).*hprim2(x0,pj2).*pj2.^(-alpha2).*w2);
